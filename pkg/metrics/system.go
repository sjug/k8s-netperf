...
func extractNodeCPU(val model.Value, nodeID string) (NodeCPU, error) {
	...
	for _, s := range v {
		...
		if strings.Contains(metric, "steal") {
			cpu.Steal = avg(s.Values)
			collected = true
		}
		if strings.Contains(metric, "system") {
			cpu.System = avg(s.Values)
			collected = true
		}
		if strings.Contains(metric, "user") {
			cpu.User = avg(s.Values)
			collected = true
		}
		if strings.Contains(metric, "nice") {
			cpu.Nice = avg(s.Values)
			collected = true
		}
		if strings.Contains(metric, "\"irq\"") {
			cpu.Irq = avg(s.Values)
			collected = true
		}
		if strings.Contains(metric, "softirq") {
			cpu.Softirq = avg(s.Values)
			collected = true
		}
		if strings.Contains(metric, "iowait") {
